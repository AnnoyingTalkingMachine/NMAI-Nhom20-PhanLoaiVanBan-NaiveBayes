...
sns.heatmap(train_confusionMatrix, annot=True, cmap='Reds', fmt='g', ax=axes[0])
axes[0].set_title("Logistic Regression, dữ liệu Train\nAccuracy: " + str(train_accuracy))
sns.heatmap(test_confusionMatrix, annot=True, cmap='Reds', fmt='g', ax=axes[1])
axes[1].set_title("Logistic Regression, dữ liệu Test\nAccuracy: " + str(test_accuracy))
...
plt.savefig("output\\LogisticRegression.png")
plt.show()
