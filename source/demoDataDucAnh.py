...
for idx, df in dataFrame.iterrows():
    try:
        if df["sentiment"] in ["enthusiasm", "surprise", "love", "fun", "happiness", "relief"]:
            data_positive.append( (tokenize_clean_sentence(df["content"]), "POS") )
        elif df["sentiment"] in ["empty", "sadness", "worry", "hate", "boredom", "anger"]:
            data_negative.append( (tokenize_clean_sentence(df["content"]), "NEG") )
        elif df["sentiment"] == "neutral":
            data_neutral.append( (tokenize_clean_sentence(df["content"]), "NEU") )
    except:
        print(idx)
